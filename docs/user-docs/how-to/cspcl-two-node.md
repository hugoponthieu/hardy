...
- If peers are not routable or discovered as expected, confirm `local-addr`/`addr` pairs are symmetric (`1<->2`) and both CLAs use the same `port`.
...
If you are rebooting often, make `vcan0` persistent with
[Persistent `vcan0` on Arch Linux](../operations/vcan-on-arch.md).
